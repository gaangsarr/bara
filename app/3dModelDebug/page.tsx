  ...
  useFrame((state, delta) => {
    if (cinematicTime < cinematicDuration) {
      ...
      const keyframes = [
        ...
        { pos: [1.0, 0.2, 5.2], lookAt: [-1.6, 0.2, 0.9], time: 0.2 },
        { pos: [2.4, 0.8, 2.3], lookAt: [0.4, -0.3, 0.3], time: 0.4 },
        { pos: [3.6, 0.1, 1.0], lookAt: [-0.1, -0.3, -2.3], time: 0.6 },
        { pos: [-7.8, 2.4, -7.9], lookAt: [0.2, 0, 0.2], time: 0.8 },
...
function CompleteScene() {
  ...
  // 🆕 DEBUG KEYBOARD LISTENER - TEKAN D untuk log pos + lookAt
  useEffect(() => {
    const handleKeyPress = (e: KeyboardEvent) => {
      if (e.key.toLowerCase() === "d") {
        const pos = camera.position;

        // Detect lookAt using raycaster (center of screen)
        ...
        if (intersects.length > 0) {
          ...
          const distance = 5;
          lookAt = [
            parseFloat((pos.x + direction.x * distance).toFixed(1)),
            parseFloat((pos.y + direction.y * distance).toFixed(1)),
            parseFloat((pos.z + direction.z * distance).toFixed(1)),
          ];
        }

        console.log("\n🎯 ================================");
        console.log("📋 Camera Keyframe:");
        console.log(
          `{ pos: [${pos.x.toFixed(1)}, ${pos.y.toFixed(1)}, ${pos.z.toFixed(
            1
          )}], lookAt: [${lookAt[0]}, ${lookAt[1]}, ${lookAt[2]}], time: 0.X }`
        );
        console.log("=================================\n");
      }
    };
  ...
  const handleClick = (event: ThreeEvent<MouseEvent>) => {
    ...
    // 🆕 DEBUG CLICK - Log click position + suggested keyframe
    ...
    console.log("\n🎯 Suggested Keyframe (looking at clicked point):");
    console.log(
      `{ pos: [${cameraPos.x.toFixed(1)}, ${cameraPos.y.toFixed(
        1
      )}, ${cameraPos.z.toFixed(1)}], lookAt: [${clickPoint.x.toFixed(
        1
      )}, ${clickPoint.y.toFixed(1)}, ${clickPoint.z.toFixed(1)}], time: 0.X }`
    );
    console.log("===================================\n");
